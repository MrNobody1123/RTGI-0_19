...
if(RT_DO_RENDER) 
{
        ...
        if(i.uv.x < 0.5) parameters.jitter = weyl3d(parameters.jitter, (qUINT::FRAME_COUNT % 3000u) * parameters.nrays + r);
        else 
        {
            parameters.jitter.xz = Random::goldenweyl2((qUINT::FRAME_COUNT % 3000u) * parameters.nrays + r, parameters.jitter.xz);
            parameters.jitter.y = Random::goldenweyl1((qUINT::FRAME_COUNT % 3000u) * parameters.nrays + r, parameters.jitter.y);
        }
...
{
    o = Denoise::filter(i, sGITexFilterTemp0, 0, 0 + SKIP_FILTER);
}
void PS_Filter1(in VSOUT i, out float4 o : SV_Target0)
{
    o = Denoise::filter(i, sGITexFilterTemp1, 1, RT_DO_RENDER + SKIP_FILTER);
}
void PS_Filter2(in VSOUT i, out float4 o : SV_Target0)
{
    o = Denoise::filter(i, sGITexFilterTemp0, 2, RT_DO_RENDER + SKIP_FILTER);
}
void PS_Filter3(in VSOUT i, out float4 o : SV_Target0)
{
    o = Denoise::filter(i, sGITexFilterTemp1, 3, RT_DO_RENDER + SKIP_FILTER);
}
...
void PS_Probe(in VSOUT i, out float4 o : SV_Target0)
{
    ...
    for(int x = 0; x < search_size_spatial; x++)
    {
        ...
        {
            float3 tapn = tex2Dlod(sGBufferTex, float4(grid_pos, 0, 0)).xyz;
            float4 tapc = tex2Dlod(sColorTex, float4(grid_pos, 0, 0));
            float similarity = pow(saturate(dot(tapn, -n)), 10.0); //reduces intensity
            similarity *= tapc.a; //mask sky
            //similarity = saturate(dot(tapn, -n) * tempF1.x - tempF1.x + 1.0); //-> use for rotation reconstruction!
            unpack_hdr(tapc.rgb);  
